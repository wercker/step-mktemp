...
if [ "$WERCKER_MKTEMP_DIRECTORY" = "true" ] ; then
  success "successfully created temporary directory at $temp_path and stored it in $WERCKER_MKTEMP_ENVVAR"
else
  success "successfully created temporary file at $temp_path and stored it in $WERCKER_MKTEMP_ENVVAR"
fi
